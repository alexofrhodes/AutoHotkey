	...
	*/
	class EventsHandler extends Dock.HelperFunc
	{
		...
		*/
		...
		{
			; Return this.WinSetTop(hwnd) ;-- original
			Return this.WinSetTop(this.hwnd.client)  ;<-- test
		}
		...
		*/
		EVENT_OBJECT_LOCATIONCHANGE(self, via)
		{
			...
			;Bottom Left
			If InStr(self.pos, "BL")
				{
					If (via = "host")
					{
						Return this.MoveWindow(self.hwnd.Client	 	;hwnd
									, Host.x 	+6					;x
									, Host.y +Host.H -Client.H	-6	;y
									, Client.w	  					;width
									, Client.h)	 					;height	
					}
			...
			;Bottom Right
			If InStr(self.pos, "BR")
				{
					If (via = "host")
					{
						Return this.MoveWindow(self.hwnd.Client	 	;hwnd
									, Host.x +host.w - client.w	-6	;x
									, Host.y +Host.H -Client.H	-6	;y
									, Client.w	  					;width
									, Client.h)	 					;height	
					}
